...
inopeger  :: i, it, iv, p1, p2, p3, t1, t2, t3, e1, e2
real(dp) :: u, v, a, lam, sx1, sx2, sx3, sy1, sy2, sy3, &
            ds1, ds2, ds3, D(nop), dqc(nvar,nop), &
...
do i=1,nof
   ...
   do iv=1,nvar
      qa(iv) = 0.5d0*(pt(t1)%qp(iv) + pt(t2)%qp(iv))
   enddo
   r   = qa(1)
   u   = qa(2)
   v   = qa(3)
   ...
   lam =  dabs(u*sx + v*sy) + a
   lam = 0.5d0*omega*lam*ds
   D(t1) = D(t1) + lam
   D(t2) = D(t2) + lam
